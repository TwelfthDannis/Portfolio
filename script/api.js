...
async function getRepositories() {
    try {
        const response = await fetch("https://pdamgupfeslxupniapku.supabase.co/rest/v1/repositories?select=*", { method: "GET", headers: { "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBkYW1ndXBmZXNseHVwbmlhcGt1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDU4MTYyOTksImV4cCI6MjAyMTM5MjI5OX0.9YF1m5roAJciaXBtJNOp60DxBGvNHL50fahbKMv4jnU" } });
        const data = await response.json();
        const rep = document.getElementById("project");
        rep.innerHTML = data.map(project => `
